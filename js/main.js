...
document.getElementById("text_input").addEventListener("keypress", async function(event) {
    if (event.key === "Enter") {
        ...
        event.target.value = "";
        await addArtist(inputText, sessionStorage.getItem("accessToken"));
    }
});
...
async function generate() {
    ...
    let profile = await getProfile();
    let playlistName = "SpotiGen Playlist: "
    for (let i = 0; i < artistList.length; i++) {
    ...
    const data = {
        name: playlistName,
        description: "Playlist generated using " + url.split("/")[2] + ", a website built and designed by Evan O'Keefe to automatically generate spotify playlists.",
        public: false
    };
